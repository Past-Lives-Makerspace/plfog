...
def dispatch(
    ...
    *,
    ...
    Pass ``suppress_email=True`` when the caller has already sent (or will send)
    a richer dedicated email for this event through another path, so this
    dispatch contributes only the in-app row and browser push. This prevents a
    double-send: the dedicated email plus the generic email this fan-out would
    otherwise produce for users opted into the trigger.
    """
    ...
    for user in user_list:
        ...
        if wants_email and not suppress_email and user.email:
            send_email(
                to=user.email,
                subject=title,
