...
def describe_seed_wiki_machine_pages():
    ...
    def describe_a_first_run():
        ...
        def it_reports_what_it_did(db):
            EquipmentFactory(name="Table Saw")
            assert "Seeded 1 machine pages: 1 added, 0 adopted, 0 refreshed, 0 skipped" in _run()
    ...
    def describe_a_second_run():
        ...
        def it_reports_the_page_as_skipped(db):
            EquipmentFactory(name="Table Saw")
            _run()
            assert "0 added, 0 adopted, 0 refreshed, 1 skipped" in _run()
    ...
    def describe_a_page_a_member_has_edited():
        ...
        def it_reports_the_page_as_skipped(db, edited):
            assert "0 added, 0 adopted, 0 refreshed, 1 skipped" in _run()
        ...
        def it_does_not_overrule_a_guild_the_page_already_has(db, edited):
            tool, page = edited
    ...
    def describe_an_archived_page_for_a_live_tool():
        ...
        def it_is_reported_as_skipped(db):
            tool = EquipmentFactory(name="Table Saw")
    ...
    def describe_dry_run():
        ...
        def it_reports_an_edited_page_as_skipped_without_touching_it(db):
            EquipmentFactory(name="Table Saw")
            _run()
            page = WikiPage.objects.get()
            page.apply_edit(editor=MemberFactory(), editor_may_verify=False, title="Mine", body="<p>Mine.</p>")
            assert "0 added, 0 adopted, 0 refreshed, 1 skipped" in _run(dry_run=True)
            page.refresh_from_db()
