...
_STATEMENT_START_RE = re.compile(r"^\s*try\b")


def _statement_handlers(text: str) -> list[int]:
    """Line numbers (1-based) of Alpine handler attributes whose value starts with ``try``."""
    offenders = []
...
def describe_alpine_handler_attributes():
    def it_never_starts_a_handler_with_a_try_statement():
        ...
        assert not offenders, (
            "An Alpine handler is an expression; a bare `try { … }` is a SyntaxError on every "
            'render. Wrap it: @click="(() => { try { $el.showPicker() } catch (e) {} })()"\n  ' + "\n  ".join(offenders)
        )
    ...
    def it_actually_detects_a_statement_handler():
        ...
        assert _statement_handlers(leaky) == [1, 2, 5]
