...
def describe_EventDelivery():
    def describe_uniqueness():
        def it_enforces_the_four_part_unique_key():
            EventDelivery.objects.create(
                event_key="site_announcement", target_ref="user:1", channel="in_app", period=""
            )
        ...
        def it_allows_the_same_target_on_a_different_channel():
            ...
            EventDelivery.objects.create(event_key="site_announcement", target_ref="user:1", channel="email", period="")
    ...
    def describe_str():
        def it_renders_a_readable_label_for_one_shot():
            row = EventDelivery(event_key="site_announcement", target_ref="user:7", channel="in_app", period="")
            assert str(row) == "site_announcement→user:7[in_app]"
