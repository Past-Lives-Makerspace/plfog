...
def describe_webhook_for_event_db_routing():
    def it_falls_back_to_the_global_webhook_with_no_route(global_webhook):
        assert discord_module.webhook_for_event("site_announcement") == global_webhook

    def it_uses_an_enabled_routes_url_over_the_global(global_webhook):
        DiscordWebhookRoute.objects.create(event_key="site_announcement", webhook_url=_ROUTED, is_enabled=True)
        assert discord_module.webhook_for_event("site_announcement") == _ROUTED
    ...
    def describe_an_enabled_route_with_a_blank_url():
        def it_silences_discord_for_that_event(global_webhook):
            ...
            DiscordWebhookRoute.objects.create(event_key="site_announcement", webhook_url="", is_enabled=True)
            assert discord_module.webhook_for_event("site_announcement") == ""

    def describe_a_disabled_route():
        def it_is_ignored_and_falls_back_to_global(global_webhook):
            DiscordWebhookRoute.objects.create(event_key="site_announcement", webhook_url=_ROUTED, is_enabled=False)
            assert discord_module.webhook_for_event("site_announcement") == global_webhook
...
def describe_save_routing():
    def it_creates_a_route_from_a_submitted_url():
        route = DiscordWebhookRoute.save_routing(
            event_key="site_announcement", submitted_url=_ROUTED, is_enabled=True, editor=None
        )
        assert route.webhook_url == _ROUTED
        assert route.is_enabled is True
        assert DiscordWebhookRoute.objects.filter(event_key="site_announcement").count() == 1

    def it_keeps_the_existing_url_when_the_submission_is_blank():
        DiscordWebhookRoute.objects.create(event_key="site_announcement", webhook_url=_ROUTED, is_enabled=True)
        route = DiscordWebhookRoute.save_routing(
            event_key="site_announcement", submitted_url="", is_enabled=False, editor=None
        )
    ...
    def it_replaces_the_url_when_a_new_one_is_submitted():
        ...
        route = DiscordWebhookRoute.save_routing(
            event_key="site_announcement", submitted_url=new_url, is_enabled=True, editor=None
        )
    ...
    def it_records_the_editor_when_one_has_a_pk():
        ...
        route = DiscordWebhookRoute.save_routing(
            event_key="site_announcement", submitted_url=_ROUTED, is_enabled=True, editor=editor
        )
    ...
    def it_drops_an_editor_without_a_pk():
        ...
        route = DiscordWebhookRoute.save_routing(
            event_key="site_announcement", submitted_url=_ROUTED, is_enabled=True, editor=User()
        )
