...
def describe_the_registry_wiring():
    @pytest.mark.parametrize("key", ["wiki.page_reported", "wiki.page_verified"])
    ...
    def it_defaults_both_to_email_as_well_as_the_bell():
        for key in ("wiki.page_reported", "wiki.page_verified"):
...
def describe_the_curated_copy():
    ...
    def it_links_the_subject_noun_in_both_emails():
        for key in ("wiki.page_reported", "wiki.page_verified"):
            entry = copy.event_copy(key)
