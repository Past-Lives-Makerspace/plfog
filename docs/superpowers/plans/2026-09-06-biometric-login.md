...
- A **spent secret coming back** is caught, at any depth of staleness, and revokes the
  credential.

  > **Superseded below.** As shipped in #333 this held only while the copy was ONE
  > rotation stale, because `previous_secret_hash` was a single slot and the credential
  > was looked up by the rotating secret itself. The follow up,
  > `2026-09-06-biometric-exact-replay-detection.md`, split the token into a stable
  > `selector` and a rotating `verifier` and closed that. Everything below this line
  > describing `secret_hash` / `previous_secret_hash` as the lookup keys is the original
  > design, kept as the record of what #333 built; the model in `core/models.py` is the
  > current truth.
