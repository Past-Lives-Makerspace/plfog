...
| **Classes' "nice and elegant" Discord presence** | A **weekly "Classes This Week" digest** — one embed to a Discord channel each Monday morning, listing the week's `upcoming_public()` sessions with `BOOK_BASE_URL` links (§5.5). Push-based *discovery*, distinct from the existing per-publish `class_published` alert and the pull-based `/whats-on`. |
| **Studio hours → Discord Scheduled Events** | **No.** `EventType.STUDIO_HOURS` rows are excluded — they are ambient standing hours (they already never announce; `announce()` no-ops for them at `membership/models.py:3623`). A dozen guilds' weekly studio hours would flood Discord's Events list; this is the one place the earlier "surface studio hours everywhere" call is reversed, because Discord's Events UI is scarcer than an in-app card. |
| **Member vs public calendar** | **Push all published events regardless of `google_calendar_target`** (simplest). Discord has one server; a Scheduled Event is GUILD_ONLY — visible to *everyone in the server*, including any non-member guests. This is an **informed visibility trade-off**, recorded in §10; the alternative (gate Discord to `PUBLIC`-target events) is a §10 open question. |
...
5. **Housekeeping.** Bump `plfog/version.py` `VERSION` (from `0.22.8`) and add **one grouped, member-facing CHANGELOG entry** — this is a net-new member-facing feature on the unreleased `0.22` line, so a **new entry at the top** (not folded into an existing one), re-stamped to the new `VERSION`. Suggested copy:
   > **Events & classes in Discord** — Community events now show up right in the Past Lives Discord server's Events tab, so you can RSVP and get reminders without leaving Discord. And every Monday, a "Classes This Week" post drops in Discord with links to sign up.
...
1. **Member-calendar events are visible to the whole Discord server.** A Scheduled Event is GUILD_ONLY = everyone in the server, including non-member guests. Locked as "push all published events." **Confirm** that's acceptable, or choose the alternative: gate Discord push to `google_calendar_target == PUBLIC` events only (add `& Q(google_calendar_target=PUBLIC)` to the push guard + `needs_discord_push`).
2. **Five `discord_*` fields on `CommunityEvent`** — the Google parallel plus `discord_pushed_occurrence`. Alternative to the extra field: on roll-forward, GET the live Discord event and compare its `scheduled_start_time` (one API call per unmappable event per night) instead of storing the occurrence. Recommend the field (cheaper, explicit).
3. **`retry_discord_event_pushes` doing double duty** (retry + roll-forward) — parity with `retry_calendar_pushes` argues for the name; if the roll-forward feels distinct, split it into `roll_discord_recurring_events`. Recommend one command.
4. **`DEFAULT_LOCATION` for a blank `event.location`** — `"Past Lives Makerspace"` constant vs. a configurable `SiteConfiguration` value. Recommend the constant (YAGNI); revisit if events span venues.
5. **Digest weekday + channel** — hardcoded **Monday** + the global webhook (routable to `#classes` via `DiscordWebhookRoute` later). Confirm Monday; a configurable weekday/channel is deferred.
