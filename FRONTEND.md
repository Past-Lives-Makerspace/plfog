...
**A view that answers some requests with a fragment must use `core.htmx.wants_fragment`**, not a hand-written header check. `HX-Request` alone is not enough (a boosted navigation carries it too), and excluding `HX-Boosted` is only half the answer: since Back is a refetch, a history restore is *precisely* a non-boosted `HX-Request`, so a check that only excludes boosted requests hands the member a bare fragment where their page used to be. That shipped once on the public class catalog: 341,276 bytes with a document, 339 without. A restore is a GET, so `@require_POST` views cannot be reached this way.
...
14. **Native `<input type="date">` / `<input type="time">` need dark-mode help.** The browser's picker icon (`::-webkit-calendar-picker-indicator`) is black by default — invisible on the dark theme. Invert it (`filter: invert(1)`) and reset it under `[data-theme="light"]` (`filter: none`). Also let the whole field open the picker, not just the icon: `@click="(() => { try { $el.showPicker() } catch (e) {} })()"` (the `try/catch` swallows the harmless "already open" error thrown when the icon itself is clicked). An Alpine handler is an **expression**, so a bare `try { … }` statement is a SyntaxError (`Unexpected token 'try'`) on every render; the arrow IIFE is what makes it one. `tests/alpine_expression_lint_spec.py` fails the suite on a handler that starts with `try`. (This was the session-scheduler date/time/Duration popover, and then issue #378.)
...
23. **A static script that registers an Alpine component loads once from `hub/base.html`'s `<head>`, deferred, before Alpine, and registers on whichever side of `alpine:init` it lands; htmx and Alpine are loaded once by the base and never again by a page.** Read *Scripts under hx-boost* before adding any `<script>` to a hub page: a body script re-runs on every boosted navigation, a head script runs once, and a component registered after Alpine has initialised the page (`cardFocus is not defined`) is dead until a hard refresh. (This is what broke the composer's card focus widget on every in-app arrival, issue #378.)
